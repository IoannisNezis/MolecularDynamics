...
void find_timestep() {
    ...
    const double lattice_constant = 2.6;
    ...
    if (is_main_rank) {
        ...
        data1 = std::ofstream(   "final/simulation_1/output/data/cutoff_Energy_Time.dat");

    }
    for (double EAM_cutoff = 10; EAM_cutoff >= 4; EAM_cutoff -= 0.5) {

        if (is_main_rank) {
            std::cout << "Testing EAM-cutoff: " << EAM_cutoff << " fs" << std::endl;
            create_lattice(cube_size, cube_size, cube_size, lattice_constant, padding);
            data2 = std::ofstream(   "final/simulation_1/output/data/TotalEnergy_T"+ std::to_string(EAM_cutoff) +".dat");
        }
        ...
        double e_max = -1000, e_min=1000, e_pot=0;
        ...
        for (int i = 0; i < 1000; ++i) {
            ...
            domain.update_ghosts(atoms, EAM_cutoff * 2);
            neighbor_list.update(atoms);
            ducastelle(atoms,domain.nb_local(), neighbor_list, EAM_cutoff);
        ...
        if (is_main_rank) {
            ...
            data1 << EAM_cutoff << " " << e_max-e_min << " "<<duration.count()/float(iterations) <<std::endl;
            std::cout << "Energy fluctuation: " << e_max-e_min << " eV" << std::endl;
            std::cout << "Total Time spend: " << duration.count()<<" milliseconds"<< std::endl;
            std::cout << "Time spend per iteration: " << std::setw(4)  << duration.count()/float(iterations)<<" milliseconds"<< std::endl;
        }
    }
    traj.close();
    //data.close();
